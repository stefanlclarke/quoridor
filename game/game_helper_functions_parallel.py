...
def parallel_legal_checks(board, point, move, visited):
...
def get_possible_move_spaces(board, location):
    ...
    while len(currently_checking) > 0:
        for point in currently_checking:
            ...
            #for move in possible_moves:
            #    print(move)
            #    p = mp.Process(target=parallel_legal_checks, args=(board, point, move, visited, queue))
            #    procs.append(p)
            #    p.start()
            #move_legals = []
            #for p in procs:
            ...
            #    move_legals.append(queue.get())
            #print(move_legals)
            if len(possible_moves) > 0:
                pool = mp.Pool(len(possible_moves))
                move_legals = pool.starmap(parallel_legal_checks, [(board, point, move, visited) for move in possible_moves])
                pool.close()
                pool.join()
                print('i')
            else:
            ...
            for i in range(len(move_legals)):
                if move_legals[i]:
                    currently_checking.append(point + possible_moves[i])
