...
class ParallelTrainer:
    def __init__(self, number_other_info=2, threads=1):
        """
        Template class for training AI on Quoridor.
        """

        self.threads = threads
        self.games = [Quoridor() for _ in range(threads)]
        self.memory_1s = [Memory(number_other_info=number_other_info) for _ in range(threads)]
        self.memory_2s = [Memory(number_other_info=number_other_info) for _ in range(threads)]
        self.spbots = [[ShortestPathBot(1), ShortestPathBot(2)] for _ in range(threads)]
    ...
    def play_games(self, info=None):
        pool = mp.Pool(self.threads)
        results = pool.starmap(self.play_game, [(info, i) for i in range(self.threads)])
        pool.close()
        pool.join()
        for i in range(len(results)):
            m1 = results[i][0]
            m2 = results[i][1]
            self.memory_1s.game_log.append(m1)
            self.memory_2s.game_log.append(m2)

    def play_game(self, info=None, printing=False, thread=0):
        """
        ...
        game = self.games[thread]
        memory_1 = self.memory_1s[thread]
        memory_2 = self.memory_2s[thread]
        spbots = self.spbots[thread]
        ...
        while playing:
            ...
            if printing:
                ...
                game.print()
            new_state, playing, winner, reward, legal, moving, illegal_move_handling, checking_winner, wall_handling = game.move(move_reformatter(move, flip=flip), get_time_info=True)
            if printing:
        ...
        print(memory_1.game_log[-1][-1][0][0].device)
        raise ValueError()
        return memory_1.game_log[-1], memory_2.game_log[-1], game_processing_time, on_policy_time, off_policy_time, moving_time, illegal_move_handling_time, checking_winner_time, wall_handling_time

    def reset_memories(self):
        """
        Resets memories of both players.
        """
        for memory in self.memory_1s:
            memory.reset()
        for memory in self.memory_2s:
            memory.reset()
        self.memory_1.reset()
        self.memory_2.reset()
    ...
    def collate_memories(self):
        """
        Collects the memory from the different threads into the main memory.
        """

        for memory in self.memory_1s:
            self.memory_1.combine(memory)
        for memory in self.memory_2s:
            self.memory_2.combine(memory)
        ...
        for j in range(iterations//save_freq):
            for i in range(save_freq):
                ...
                for k in range(games_per_iter):
                    self.play_games(info=[j,i,k])
                    #game_time, on_time, off_time, m_time, ill_time, cw_time, wh_time = self.play_game(info=[j, i, k])
                    #self.log_memories()
                    self.collate_memories()
                    print(self.memory_1s[0].rewards)
                    print(self.memory_1.rewards)
                    if get_time_info:
